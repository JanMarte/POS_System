...
const RecipeModal = ({ isOpen, onClose, initialSearch }) => {
    ...
    const handleSearch = async (term) => {
        ...
        try {
            ...
            // 🛡️ SECURITY CHECK: Ensure it's actually an array
            if (data.drinks && Array.isArray(data.drinks) && data.drinks.length > 0) {
                setResults(data.drinks);
                setView('list');
            } else {
                // 2. Fallback: Search by Ingredient
                try {
                    ...
                    if (filterData.drinks && Array.isArray(filterData.drinks) && filterData.drinks.length > 0) {
    ...
    return (
        <div className="modal-overlay">
            <div className="modal-content" style={{ width: '800px', maxWidth: '95vw', height: '85vh', display: 'flex', flexDirection: 'column', padding: '20px' }}>
                ...
                {/* VIEW 1: RESULTS LIST */}
                {!loading && !error && view === 'list' && Array.isArray(results) && (
                    <>
                            ...
                            {results.map(drink => (
                                <div
                                    ...
                                    style={{
                                        ...
                                        minHeight: '220px' // 👈 Forces card to be tall enough
                                    }}
                                >
                                    {/* Image with flexShrink 0 to prevent squishing */}
                                    <img
                                        src={drink.strDrinkThumb}
                                        alt={drink.strDrink}
                                        style={{ width: '100%', height: '150px', objectFit: 'cover', flexShrink: 0 }}
                                    />
                ...
                {!loading && !error && view === 'detail' && selectedDrink && (
                    <div style={{ flex: 1, overflowY: 'auto', textAlign: 'left', paddingRight: '5px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '20px', marginBottom: '20px', alignItems: 'start' }}>
                            <img src={selectedDrink.strDrinkThumb} alt={selectedDrink.strDrink} style={{ width: '150px', height: '150px', borderRadius: '10px', objectFit: 'cover', border: '2px solid #444' }} />
                            <div>
